...
def permutation(context, initial_index, current_index, next_index_list, item_count, tot_pathlen, tot_weight):
    ...
    for i in next_index_list:  # 枚举下一个目标物品
        ...
        if itemspan[i] < tot_pathlen + (itemgragh[current_index][i] if current_index < i else itemgragh[i][current_index]):
...
def update(context):
    global pre_pos, pre_item_count, is_leaving, is_stuck, itemlist, itemlen, itemgragh, itemspan, item_weight, pre_dst_list, pre_pos_list, head, tail, last_dst, extra_obstacles_list, extra_obstacles_list, exit_pathlen
    me = context.me
    rival = context.players[1]
    print("-----------------")
    ...
    for i in range(itemlen):
        ...
        exit_pathlen.append(len(api.check.path(
            itemlist[i], context.exit, method="bfs", player_id=stuck_check(context))) - 1)
    ...
    for i in range(itemlen):
        ...
        if pathlen == 0:
            print("----------------------------------")
            print("警告：pathlen == 0! 将随机为3-20的整数!")
    ...
    me_dis = len(api.check.path(
        me, dst, method="bfs", extra_obstacles=None if is_leaving else extra_obstacles_list, player_id=stuck_check(context))) - 1
    ...
    for i in range(4):
        ...
        new_dis = len(api.check.path(
            (new_row, new_col), dst, method="bfs", extra_obstacles=None if is_leaving else extra_obstacles_list, player_id=stuck_check(context))) - 1
        ...
        for j in range(itemlen):
            # 计算物品离自己的距离
            pathlen = len(api.check.path(
                (new_row, new_col), itemlist[j], extra_obstacles=None if is_leaving else extra_obstacles_list, method="bfs", player_id=stuck_check(context))) - 1
