...
itemspan = {}  # 存储物品“寿命”
item_weight = {}  # 存储物品权重
extra_obstacles_list = []
exit_pathlen = []
is_blocked_to_death = False
...
def should_leave(context):
    ...
    pathlen = len(api.check.path(
        context.me, context.exit, method="bfs", extra_obstacles=None if is_leaving else extra_obstacles_list)) - 1
    ...
    if context.me.energy <= pathlen + 1 + 5 and pre_item_count != context.me.item_count:
        is_leaving = True
    # 若当前体力小于等于离场最小体力+4，则强制离场
    if context.me.energy <= pathlen + 1 + 4:
        is_leaving = True
    return is_leaving
        ...
        for row in range(len(maze)):
            for col in range(len(maze[row])):
                if maze[row][col] == 'WALL':
                    print('#', end='')
                elif [row, col] in extra_obstacles_list:
                    print('S', end='')
                else:
                    print(' ', end='')
            print()
...
def calc_extra_obstacles(context):
    ...
    if is_leaving or is_blocked_to_death:
        return []
    extra_obstacles_list = []
    me = context.me
    ...
    if is_blocked_maliciously:
        # extra_obstacles_list.append([rival.row, rival.col])
        ...
        for i in range(4):
            ...
            while 0 <= new_row < MAZE_SIZE and 0 <= new_col < MAZE_SIZE and maze[new_row][new_col] != 'WALL' and not (new_row == me.row and new_col == me.col):
                ...
                for j in range(itemlen):
                    if new_row == itemlist[j].row and new_col == itemlist[j].col:
                        is_item = True
                        print("当前位置为：", j, itemlist[j])
                        break
                if is_item:
                    break
                if api.check.who_is_closer(new_row, new_col, players=None) == rival:
                    extra_obstacles_list.append([new_row, new_col])
                new_row += DRC[i][0]
                new_col += DRC[i][1]
        '''废稿
        ...
        for row in range(len(maze)):
            for col in range(len(maze[row])):
                if maze[row][col] != 'WALL' and api.check.who_is_closer(row, col, players=None) == rival:
                    extra_obstacles_list.append([row, col])
        '''

    return extra_obstacles_list


# 判断是否被卡住
def stuck_check(context):
    global is_stuck, pre_pos, pre_item_count, extra_obstacles_list
    if is_leaving:
        return -1
    ...
    if is_stuck:  # 若被卡住则将对手视为不可穿透
        extra_obstacles_list.append(
            [context.players[1].row, context.players[1].col])
...
def traverse(context, current_index, traverse_list):
    ...
    for i in traverse_list:
        pathlen = (itemgragh[current_index][i] if current_index <
                   i else itemgragh[i][current_index])
    ...
    next_index_list = [
        i for i in traverse_list if (itemgragh[current_index][i] if current_index < i else itemgragh[i][current_index]) == min_pathlen and rival.item_count[itemlist[i].name] == min_cnt]
...
def permutation(context, initial_index, current_index, next_index_list, item_count, tot_pathlen, tot_weight):
    ...
    leave_pathlen = len(api.check.path(
        itemlist[current_index], context.exit, method="bfs", extra_obstacles=None if is_leaving else extra_obstacles_list)) - 1
    '''
    me = context.me
    rival = context.players[1]

    leave_pathlen = exit_pathlen[current_index]
    ...
    if initial_index in item_weight:  # 若item_weight中已有值，则取较大者保留
        ...
        item_weight[initial_index] = tot_weight
    # 物品权重计算预处理
    nobox_item_count = [item_count[k]
                        for k in item_count if k != BOX]  # 仅考虑已捡宝石数
    if anti_blocking_mode:  # 若打开防登模式
        if context.round < TOTAL_ENERGY/5:  # 若刚开局，则前200回合根据对手分数来短视收益最大化来捡物品
            # print("现在是短视模式")

            item_count_min = min(nobox_item_count)
            if rival.score > me.score + 3:  # 若对手领先（有开堵可能性），则短视化只补一套宝石（非全套，仅填补最少几种）
                expected_remaining_gems_count = sum(
                    1 for value in nobox_item_count if value == item_count_min)
                # print("对手可能开堵！")
            else:
                i = 1
                while rival.score <= me.score - 20*i:
                    i += 1
                i += 1
                # 若对手离我方仅差i-1套宝石，则将余下收益期望定为当下再补i套所需的宝石数（非全套，仅填补最少几种）
                expected_remaining_gems_count = sum(
                    item_count_min + i - value for value in nobox_item_count if value < item_count_min + i)
                # print("补齐至", item_count_min + i, "套宝石")
        else:
    ...
    sorted_item_count = sorted(
        nobox_item_count, reverse=True)  # 将各物品数目取出并降序排序
    ...
    for i in next_index_list:  # 枚举下一个目标物品
        ...
        if itemlist[i].name == BOX:
            item_value[i] += BOX_VALUE_MEAN * BOX_MULTIPLE
        # 若当前目标是宝石
        else:
            # 将物品价值加上成套额外收益
            '''期望成套收益总和姑且先不放'''
            if item_count[itemlist[i].name] < gem_optimal_quantity:
                item_value[i] += GEM_SET * ((gem_optimal_quantity -
                                            item_count[itemlist[i].name]) / expected_remaining_gems_count)**GEM_MI
        ...
        ###
        if tot_pathlen == 0:
            print("警告！tot_pathlen == 0!")
            # print("当前遍历物品：", current_index, itemlist[current_index])
            # print("余下物品：", next_index_list)
        if (itemgragh[current_index][i] if current_index < i else itemgragh[i][current_index]) <= 0:
            print("警告！局部路径长 <= 0!")
            # print("当前遍历物品：", current_index, itemlist[current_index])
            # print("出错距离的另一物品为：", i, itemlist[i])
            # print("余下物品：", next_index_list)
            # print("路径长为：", (itemgragh[current_index][i] if current_index < i else itemgragh[i][current_index]))
            # print("原始itemgragh：", itemgragh[current_index][i], itemgragh[i][current_index])

        # 若路径异常 或 该物品在自己获取之前可能已被对手获取，则不将该物品计入item_count、tot_pathlen和tot_weight（但new_next_index_list需更新）
        if (itemgragh[current_index][i] if current_index < i else itemgragh[i][current_index]) <= 0:
            permutation(context, initial_index, current_index, new_next_index_list,
                        item_count, tot_pathlen, tot_weight)
        elif itemspan[i] < tot_pathlen + (itemgragh[current_index][i] if current_index < i else itemgragh[i][current_index]):
            permutation(context, initial_index, current_index, new_next_index_list,
                        item_count, tot_pathlen, tot_weight)
        # 否则记为获取
        else:
            permutation(context, initial_index, i, new_next_index_list, new_item_count, tot_pathlen +
                        (itemgragh[current_index][i] if current_index < i else itemgragh[i][current_index]), tot_weight + item_value[i] / tot_pathlen)


def update(context):
    global pre_pos, pre_item_count, is_leaving, is_stuck, itemlist, itemlen, itemgragh, itemspan, item_weight, pre_dst_list, pre_pos_list, head, tail, last_dst, extra_obstacles_list, extra_obstacles_list, exit_pathlen, is_blocked_to_death
    me = context.me
    rival = context.players[1]
    print("-----------------")
    ...
    print("is_blocked_maliciously: ", is_blocked_maliciously)
    ...
    extra_obstacles_list = calc_extra_obstacles(context)

    stuck_check(context)
    print("是否被堵：", is_stuck)

    if itemless_rounds >= 300:
        is_blocked_to_death = True
        is_leaving = True

    if itemless_rounds == 0:
        is_blocked_to_death = False

    if is_blocked_to_death:  # 如果被堵死，直接把目标定为出口
        dst = context.exit
    # 若处在孔雀东南飞状态，则强制将目标设为上一个目标
    elif pacing_check(context) == True:
        dst = last_dst
        ...
        for i in range(itemlen):
            ...
            for j in range(i+1):
                tmp.append(-1000)
            # 遍历itemlist, 计算从当前物品到每一个物品的距离（注：Path方法计算出来的长度包括起点和终点，故-1；为了减少计算量，仅计算从小下标到大下标的距离）
            for j in range(i+1, itemlen):
                tmp.append(len(api.check.path(
                    itemlist[i], itemlist[j], method="bfs", extra_obstacles=None if is_leaving else extra_obstacles_list)) - 1)
            # 计算从当前物品到出口的距离
            exit_pathlen.append(len(api.check.path(
                itemlist[i], context.exit, method="bfs", extra_obstacles=None if is_leaving else extra_obstacles_list)) - 1)
        ...
        if rival.energy == 0:
            ...
            leave_pathlen = len(api.check.path(
                context.me, context.exit, method="bfs", extra_obstacles=None if is_leaving else extra_obstacles_list)) - 1
            nobox_item_count = [me.item_count[k]
                                for k in me.item_count if k != BOX]  # 仅考虑已捡宝石数
            if anti_blocking_mode:  # 若打开防登模式
                if context.round < TOTAL_ENERGY/5:  # 若刚开局，则前200回合根据对手分数来短视收益最大化来捡物品
                    print("现在是短视模式")
                    item_count_min = min(nobox_item_count)
            ...
            print("期望可捡宝石数：expected_remaining_gems_count: ",
                  expected_remaining_gems_count)

            sorted_item_count = sorted(
                nobox_item_count, reverse=True)  # 将各物品数目取出并降序排序
            # print("我捡的物品（me.item_count）: ", me.item_count)
            # print("去除箱子后的item_count（nobox_item_count_values）: ", nobox_item_count)
            # print("排序后的item_count（sorted_item_count）: ", sorted_item_count)
            # 求解各宝石最大收益数量
            gem_optimal_quantity = 0  # 即E(Cm)
            for i in range(5):
                cnt = 0
            ...
            print("gem_optimal_quantity: ",
                  gem_optimal_quantity)
            expected_remaining_gem_set_score = GEM_SET * \
                (gem_optimal_quantity - min(sorted_item_count)
                 )   # 求余下期望成套收益总和 即E(gem_set)
            # 开始遍历
            item_value = [0]*itemlen
            for i in range(itemlen):
                # #处理item_value#
                ...
                # 若当前目标是宝石
                    ...
                    if me.item_count[itemlist[i].name] < gem_optimal_quantity:
                        '''期望成套收益总和姑且先不放'''
                        item_value[i] += GEM_SET * ((gem_optimal_quantity -
                                                    me.item_count[itemlist[i].name]) / expected_remaining_gems_count)**GEM_MI
                    # print("物品名称itemlist[i].name: ", itemlist[i].name,
                    #       "物品数量item_count[itemlist[i].name]: ", me.item_count[itemlist[i].name],
                    #       "物品期望收益item_value[i]: ", item_value[i])
                # 计算物品离自己的距离
                pathlen = len(api.check.path(
                    me, itemlist[i], method="bfs", extra_obstacles=None if is_leaving else extra_obstacles_list)) - 1
                if pathlen <= 0:
                    print("----------------------------------")
                    print("警告：pathlen <= 0!")
                    print("当前正在计算与物品", itemlist[i], "的距离")
                    print("----------------------------------")
                    # pathlen = random.randint(3, 20)
                    continue

                # 若该物品在自己获取之前可能已被对手获取，则不予考虑
                if i in itemspan and itemspan[i] < pathlen:
                    continue
    ...
    drc_weight = [0]*4
    # 计算自己到目标物品的距离
    print("计算me_dis时的额外障碍：", extra_obstacles_list)
    me_dis = len(api.check.path(
        me, dst, method="bfs", extra_obstacles=None if is_leaving else extra_obstacles_list)) - 1
    # 若当前目标物品处在额外障碍范围内（即距离计算结果为-1），则随机选择新目标物品
    if me_dis == -1 and len(itemlist) != 0:
        item = random.choice(itemlist)
        while item == last_dst:  # 防止与当前相同
            item = random.choice(itemlist)
        last_dst = dst = item
    ...
    for i in range(4):
        ...
        if context.maze[new_row][new_col] == 'WALL':
            ...
            continue
        # 若该方向在额外障碍范围内，则不予考虑
        if is_leaving == False and [new_row, new_col] in extra_obstacles_list:
            print("当前方向为额外障碍")
            continue
        # 若当前已被卡住且该方向为对手则不予考虑
        ...
        new_dis = len(api.check.path(
            (new_row, new_col), dst, method="bfs", extra_obstacles=None if is_leaving else extra_obstacles_list)) - 1
        ...
        for j in range(itemlen):
            # 计算物品离自己的距离
            pathlen = len(api.check.path(
                (new_row, new_col), itemlist[j], extra_obstacles=None if is_leaving else extra_obstacles_list, method="bfs")) - 1
