...
###
GEM_MUTIPLE = 200  # 局尾宝石成套收益权重up
ENERGY_THRESHOLD = TOTAL_ENERGY / 10    # 进入局尾的判定条件
###
...
def permutation(context, initial_index, current_index, next_index_list, item_count, tot_pathlen, tot_weight):
    ...
    nobox_item_count_values = [item_count[k] for k in item_count if k != BOX]
    sorted_item_count = sorted(
        nobox_item_count_values, reverse=True)  # 将各物品数目取出并降序排序
    ...
    for i in next_index_list:  # 枚举下一个目标物品
        ...
        # 若当前目标是宝石
            ...
            if item_count[itemlist[i].name] < gem_optimal_quantity:
                item_value[i] += GEM_SET * (GEM_MUTIPLE if context.me.energy < ENERGY_THRESHOLD else 1) * ((gem_optimal_quantity -
...
def update(context):
    ...
    nobox_item_count_values = [me.item_count[k]
                               for k in me.item_count if k != BOX]
    sorted_item_count = sorted(
        nobox_item_count_values, reverse=True)  # 将各物品数目取出并降序排序
    print("我捡的物品（me.item_count）: ", me.item_count)
    print("去除箱子后的item_count（nobox_item_count_values）: ", nobox_item_count_values)
    ...
    for i in range(itemlen):
        ...
        # 若当前目标是宝石
            ...
            if me.item_count[itemlist[i].name] < gem_optimal_quantity:
                '''期望成套收益总和姑且先不放'''
                item_value[i] += GEM_SET * (GEM_MUTIPLE if me.energy < ENERGY_THRESHOLD else 1) * ((gem_optimal_quantity -
                                            me.item_count[itemlist[i].name]) / expected_remaining_gems_count)**GEM_MI
            print("宝石倍数：", GEM_MUTIPLE if context.me.energy < ENERGY_THRESHOLD else 1)
            # print("物品名称itemlist[i].name: ", itemlist[i].name,
