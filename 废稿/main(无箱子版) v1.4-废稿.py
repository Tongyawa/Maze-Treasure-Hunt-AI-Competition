...
def should_leave(context):
    ...
    if context.me.energy <= pathlen + 1:
        is_leaving = True
    return is_leaving
...
pre_dst_list = []
pre_pos_list = []
head = tail = 0  # 1为pre_dst_list，2为pre_pos_list
...
def calc_extra_obstacles(context):
    ...
    if is_blocked_maliciously:
        ...
        for i in range(4):
            ...
            while maze[new_row][new_col] != 'WALL' and not (new_row == me.row and new_col == me.col):
                # 若对手能比己方先到
...
def stuck_check(context):
    global is_stuck, pre_pos, pre_item_count, is_ahead
    if pre_pos == (context.me.row, context.me.col):
        is_stuck = True
    elif context.me.item_count != pre_item_count:  # 若刚捡到宝石，则取消卡住状态
        is_stuck = False
    if context.me.score > context.players[1].score + 5:
        is_ahead = True
    else:
        is_ahead = False
    if is_stuck and not is_ahead:  # 若被卡住 且 分数低于对手，则将对手视为不可穿透
        return 0
...
# 计算对手正在移向的目标
def calculate_closer_items(context, pre_rival_pos, block_flag):
    closer_list = []
    for i in range(itemlen):
        pre_pathlen = len(api.check.path(
            pre_rival_pos, itemlist[i], method="bfs", player_id=block_flag)) - 1
        pathlen = len(api.check.path(
            context.players[1], itemlist[i], method="bfs", player_id=block_flag)) - 1
        if pathlen < pre_pathlen:
            closer_list.append(i)
    return closer_list
...
def traverse(context, current_index, traverse_list):
    ...
    for i in next_index_list:
        ...
        itemspan[i] = min(itemspan[i], itemspan[current_index] + min_pathlen)
...
def update(context):
    global pre_pos, pre_item_count, is_leaving, is_stuck, itemlist, itemlen, itemgragh, itemspan, item_weight, pre_dst_list, pre_pos_list, head, tail, last_dst, rival_dst_list, pre_rival_item_count, pre_rival_pos
    me = context.me
    rival = context.players[1]
    print("-----------------")
    ...
    itemspan = [99999 for x in range(itemlen)]  # 存储物品“寿命”
    ...
    if rival.energy == 0:
        ...
        if rival.item_count != pre_rival_item_count:    # 若对手刚获取到物品（含刚开局），则将rival_dst_list重置为全体物品
            rival_dst_list = [x for x in range(itemlen)]
        if pre_rival_pos != None:   # 若不为开局（pre_rival_pos不为None）则更新rival_dst_list
            new_rival_dst_list = []
            closer_items = []
            closer_items.append(calculate_closer_items(
                context, pre_rival_pos, 1))  # 我方不可穿透
            closer_items.append(calculate_closer_items(
                context, pre_rival_pos, -1))  # 我方可穿透
            print("对手本回合移向物品（我方不可穿透）：", closer_items[0])
            print("对手本回合移向物品（我方可穿透）：", closer_items[1])
            if closer_items[0] != closer_items[1]:
                print("调试信息：对手移向物品两种计算方式结果不同。")
            for i in closer_items[0]:  # 我方不可穿透
                if i in rival_dst_list and i not in new_rival_dst_list:
                    new_rival_dst_list.append(i)
            for i in closer_items[1]:  # 我方可穿透
                if i in rival_dst_list and i not in new_rival_dst_list:
                    new_rival_dst_list.append(i)
            rival_dst_list = copy.deepcopy(new_rival_dst_list)
        print("rival_dst_list：", rival_dst_list)

        # 从对手可能的目标中寻找离对手最近且对手持有最少的物品并计算距离
        for i in rival_dst_list:
            pathlen = len(api.check.path(
    ...
    me_dis = len(api.check.path(
        me, dst, method="bfs", extra_obstacles=calc_extra_obstacles(context), player_id=stuck_check(context))) - 1
    ...
    for i in range(4):
        ...
        if context.maze[new_row][new_col] == 'WALL':
            ...
            continue
        # 若该方向在额外障碍范围内，则不予考虑
        if [new_row, new_col] in calc_extra_obstacles(context):
            print("当前方向为额外障碍")
            continue
        # 若当前已被卡住且该方向为对手则不予考虑
        ...
        new_dis = len(api.check.path(
            (new_row, new_col), dst, method="bfs", extra_obstacles=calc_extra_obstacles(context), player_id=stuck_check(context))) - 1
        ...
        for j in range(itemlen):
            # 计算物品离自己的距离
            pathlen = len(api.check.path(
                (new_row, new_col), itemlist[j], extra_obstacles=calc_extra_obstacles(context), method="bfs", player_id=stuck_check(context))) - 1
    ...
    pre_rival_pos = (rival.row, rival.col)
    pre_item_count = me.item_count
    pre_rival_item_count = rival.item_count
    last_dst = dst
    return direction
