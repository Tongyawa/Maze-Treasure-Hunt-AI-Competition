...
def pacing_check(context):
    global is_pacing, pre_dst, head, tail, itemless_rounds, last_dst
    if tail >= 4:
        # 第一重特判: 若连续4回合的目标物品在反复横跳（ABAB），则判定为孔雀东南飞
        if pre_dst[head] == pre_dst[head+2] and pre_dst[head+1] == pre_dst[head+3] and pre_dst[head] != pre_dst[head+1]:
            is_pacing = True
    if context.me.item_count != pre_item_count:  # 若刚获取到物品，则重置未获取物品回合数 以及 消除孔雀东南飞状态
        itemless_rounds = 0
        is_pacing = False
    else:
        itemless_rounds += 1
    # 第二重特判: 若连续20回合以上没获取到物品，则判定为孔雀东南飞，且切换目标为目标记录队列中最近一个与当前目标不同的目标；
    # 为了防止切换目标自己导致孔雀东南飞，特此将为捡到物品回合重置
    if itemless_rounds >= 20:
        is_pacing = True
        itemless_rounds = 0
        # 随机选择
    ...
    print("pre_dst(最近几个目标): ", pre_dst[head:tail])
...
def stuck_check(context):
    ...
    if pre_pos == (context.me.row, context.me.col):
        is_stuck = True
    if pre_pos == True and context.me.itemcount == pre_item_count:
        is_stuck = False
...
def permutation(context, initial_index, current_index, next_index_list, item_count, tot_pathlen, tot_weight):
    ...
    for i in next_index_list:  # 枚举下一个目标物品
        ...
        print("itemgragh[current_index][i]: ", itemgragh[current_index][i])
...
def update(context):
    global pre_pos, pre_item_count, is_leaving, is_stuck, itemlist, itemlen, itemgragh, itemspan, item_weight, pre_dst, head, tail, last_dst
    me = context.me
    ...
    if rival.energy == 0:
        ...
        for i in range(itemlen):
            ...
            min_pathlen = min(min_pathlen, pathlen)
        for i in range(itemlen):
            pathlen = len(api.check.path(
                rival, itemlist[i], method="bfs", player_id= -1 )) - 1
    ...
    for i in range(itemlen):
        ...
        print("计算自己到物品的路径：", itemlist[i].name, api.check.path(
            me, itemlist[i], method="bfs", player_id=stuck_check(context)))

        pathlen = len(api.check.path(
            me, itemlist[i], method="bfs", player_id=stuck_check(context))) - 1
        
        print("计算物品离自己的距离pathlen: ", pathlen)
    ...
    me_dis = len(api.check.path(
        me, dst, method="bfs", player_id=stuck_check(context))) - 1
    # 计算各个方向的泛化期望收益权重
    for i in range(4):
        ...
        new_dis = len(api.check.path(
            (new_row, new_col), dst, method="bfs", player_id=stuck_check(context))) - 1
        ...
        for j in range(itemlen):
            # 计算物品离自己的距离
            pathlen = len(api.check.path(
                (new_row, new_col), itemlist[j], method="bfs", player_id=stuck_check(context))) - 1
